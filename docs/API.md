...
**End of song belongs to the server.** `server_eos: true` means the server owns completion: the browser backend uses the stored clip duration plus ~1s of grace for client start latency, while `AUDIO_BACKEND=server` watches mpv EOF and then gives browser listeners ~1s to finish. Clients **must not** report `ended`, and a matching play-ID-bearing automatic stop is ignored — with several devices opted in to play, the first one to finish would otherwise stop the clip for all the others, and with no player-role client connected nothing would report at all. `server_eos: false` is limited to a browser-backend legacy row with no stored `duration_sec`; there is no server-side end detection for that play, so a player-role client still reports `ended`.
...
Browser playback backend — client audio roles: every WS client mirrors playback state, but only a client holding the **player** role plays `audio_url` via HTMLAudioElement at `volume` (0–100 → 0–1, `volume_boost_db` via WebAudio GainNode when nonzero) and, when `server_eos` is false, reports natural end-of-song (`POST /api/playback/stop` echoing `play_id`). Every other client is a **controller**: it sends commands and renders state but never produces sound and never reports `ended`. On `stop`, a player halts immediately (<200ms). On connect, the server sends a `state` message.
