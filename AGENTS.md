...
- PRs: **always open as a draft** (owner directive, 2026-09-19). The free static reviewers (DeepSource, Codacy, Gitar, Sourcery's guide) run on drafts — iterate against those until they converge. The owner flips the PR to ready-for-review when it is worth the high-value reviewers' time; that is what triggers CodeRabbit and Greptile, which skip drafts. Never undraft on your own. **Exception:** a PR that changes only `AGENTS.md` (owner directive, 2026-09-19) merges straight to `main` once the free reviewers have settled — no undraft, no wait for the high-value bots.
- Bot findings: **batch them.** Collect everything a review round surfaces (all bots, all threads), push ONE fix commit, then let the bots re-run once. Four one-line fix pushes = four full re-reviews and a paused CodeRabbit.
- Static analysis: DeepSource runs on every push (`.deepsource.toml` — python, javascript, docker, secrets). Fix what it flags; for a finding that is a deliberate choice (see "Accepted by design", module-level `global` state) add a `# skipcq: <CODE> - why` comment on the flagged line rather than ignoring it in the dashboard, so the reason travels with the code.
...
- Verify via the UI at http://localhost:8080/?player=1 (kiosk grid; without `?player=1` the page is a silent controller — state updates but no audio element playback) — tap a seeded player tile (e.g. Bobby 'Rocket' Reyes #3) to play a walk-up clip. Playback state is server-side (WS `/ws`); the server ends a play itself from the clip's `duration_sec` (`server_eos`), so no client needs to report `ended` to clear "playing".
...
- 2026-07-26: Adding a sentinel row to decide "did this env var change since last boot?" needs an UPGRADE path, or its first boot lies: on every pre-existing DB the sentinel is missing, which looks identical to "brand new" and clobbers whatever the admin had set. Pair the sentinel with the "upgrade untouched defaults only" trick already used for `default_snippet_length`.
- 2026-07-26: A per-page browser role breaks the moment the operator navigates. The audio role lived in one page's module state, so tapping ADMIN on the Pi left the device with NO player (admin.html has no toggle) and the clip "Test" button was silent. Client-side role state that represents *the device* must be persisted, not just read from the URL of whichever page is open.
- 2026-07-25: `kill %1` does NOT kill headless Chrome in these test scripts — the wrapper re-execs, the job dies, the browser lives. A stale `?player=1` page from an earlier test then ran the OLD ws.js against the NEW server and reported `ended`, ending a "no listener connected" timer test 0.9s early and nearly passing off a wrong result as right. Kill browsers by PID from `pgrep -f "google-chrome[-]stable"` (the bracket keeps the pattern from matching your own shell — `pkill -f google-chrome` kills the shell running it) and confirm the count is 0 before timing anything. A later run hit this again: ten stale browsers made a log-based fetch count read 1 instead of 0.
...
- 2026-09-17/18: Rendered media is rewritten in place under a fixed URL (`/media/clips/<id>.mp3`), and Chromium's media cache reuses the already-loaded resource for that URL — `audio.load()` after `audio.src = sameUrl` refetched 0/6 times in the play-over-play sequence, and `Cache-Control: no-cache` can't help because no request is made at all. **Only a URL that changes with the file is deterministic**: `audio_url` now carries `?v=<mtime_ns>` (`db._media_url`), clients use the string verbatim. Lesson: a fix that passes once in a browser is not verified — repeat the exact failing sequence several times (headless Chromium + CDP `Network.responseReceived` shows whether any fetch happened).
- 2026-09-18: `touch-action: manipulation` on a long-press target lets Chromium start a pan on a ~12 px finger slide and fire `pointercancel` — the 600 ms home-run hold died silently (no tap either). Long-press targets that don't scroll get `touch-action: none`; rows inside a scrollable list get `pan-y`. Drive touch with CDP `Input.dispatchTouchEvent` on a `hasTouch` context to test it.
- 2026-09-18: gpiozero 2.x creates the pin factory lazily — `Device.pin_factory` is `None` in a fresh interpreter until `Device.ensure_pin_factory()` (or the first Device). A probe that reads the attribute cold always reports the buttons dead.
- 2026-09-18: Report source durations rounded DOWN (`math.floor(d*1000)/1000`): `round()` can exceed the exact ffprobe float, and the editor sends the reported number back as `trim_end_sec`, which the exact-float check then rejected with a message printing that very number.
- 2026-09-17: yt-dlp's `noplaylist` only applies to a URL naming BOTH a video and a list (`watch?v=…&list=…`); a bare `playlist?list=…` ignores it and downloads every entry (`_yes_playlist` returns True when there is no video id). Pair it with `playlist_items: "1"`. Likewise `socket_timeout` bounds one socket op, not the download — a wall-clock cap needs a progress hook that raises `DownloadCancelled`.
- 2026-09-17: `db.source_protected` is the ONE place that decides whether a `sources/` file may be unlinked. Any new "someone still needs this source" rule (clipper's live-job check went in via `db.register_source_guard`) belongs there, not in a single caller — the hourly sweep had the live-job rule and `delete_clip` didn't, so deleting a clip stranded the job it was saved from.
