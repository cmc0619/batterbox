...
- 2026-09-17: Setting `audio.src` to the SAME string does not reload the media in Chromium, and `Cache-Control: no-cache` can't help because no request is made at all. Rendered media is rewritten in place under a fixed URL (`/media/clips/<id>.mp3`), so a re-trimmed clip played stale on the kiosk until a page reload. `startAudio` now calls `audio.load()` after assigning `src`. Reproduced/verified with headless Chromium + CDP network events (no fetch at all on the second play before the fix).
